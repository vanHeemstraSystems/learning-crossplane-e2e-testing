...
# - Windows Minikube: --path=./flux/clusters/dev-win
# - Mac Minikube:     --path=./flux/clusters/dev-mac
...
### 16.1 GitOps Test: Commit a change and watch Flux + Crossplane reconcile

This section validates the full GitOps loop:

1. A file changes in GitHub
2. Flux detects the change and applies it
3. Crossplane observes the new desired state and reconciles

We provide two options:

- **Option A (config-only, safest)**: update a label on the PostgreSQL `Composition` and confirm it appears in-cluster.
- **Option B (proves Crossplane reconciliation)**: update the PostgreSQL `Composition` to add a tag on the composed Azure `ResourceGroup`, then confirm it propagates to the managed resource (and optionally Azure).

#### 16.1.1 Apply the Flux GitRepository + Kustomization to your cluster

These manifests configure Flux to sync Crossplane APIs from your Git repo:

```bash
kubectl apply -f flux/clusters/dev/crossplane-repo.yaml
kubectl apply -f flux/clusters/dev/crossplane-kustomizations.yaml

# Wait until ready
kubectl get -n flux-system gitrepositories.source.toolkit.fluxcd.io crossplane-configs
kubectl get -n flux-system kustomizations.kustomize.toolkit.fluxcd.io crossplane-apis

flux get sources git
flux get kustomizations
```

If `crossplane-apis` is not Ready, check events and logs:

```bash
kubectl describe -n flux-system kustomization crossplane-apis
kubectl logs -n flux-system deployment/kustomize-controller --tail=200
```

#### 16.1.2 Make a small change in Git (PostgreSQL Composition)

Edit `apis/v1alpha1/postgresql-databases/composition.yaml`.

**Option A (config-only)**: add (or update) a label under `metadata.labels`:

- `gitops-test: "<some-value>"`

Example value could be a timestamp: `gitops-test: "2026-01-31-001"`.

**Option B (Crossplane reconciliation)**: add a tag to the composed ResourceGroup base in the Composition (under the `ResourceGroup` `spec.forProvider.tags` block), for example:

- `gitopsTest: "true"`

Commit and push to your GitHub repo (the same repo you bootstrapped Flux from):

```bash
git add apis/v1alpha1/postgresql-databases/composition.yaml
git commit -m "GitOps test: update composition label"
git push origin main
```

#### 16.1.3 Force a reconcile (or wait for interval)

You can wait for Flux’s intervals, or reconcile immediately:
...
Verify Flux applied the new revision:

```bash
kubectl get -n flux-system kustomization crossplane-apis \
  -o jsonpath='{.status.lastAppliedRevision}'; echo
```

#### 16.1.4 Verify the change is live in the cluster

Confirm the label exists on the Composition in Kubernetes:
...
If the value matches what you committed, you have proven the end-to-end GitOps flow (Git → Flux → Kubernetes) for Crossplane configuration.

#### 16.1.5 (Optional) Verify Crossplane reconciliation happened

If you performed **Option B** (ResourceGroup tag change), confirm the composed ResourceGroup managed resource reflects the new desired state:
...
Optionally verify the tag in Azure (see also `## Verification Steps` → `### 3. Monitor with Azure CLI`):
